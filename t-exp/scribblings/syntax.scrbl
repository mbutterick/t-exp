...
@(define (t-exp-racket)
   @racket[#, @hash-lang[] #, @racketmodname[t-exp] #, @racketidfont{racket}])

@title[#:tag "reader"]{T-expression syntax}

The T-expression notation is designed to be a convenient facility for
free-form text in Racket code. The prefix character ``@"@"''
was chosen because it's among the least-used characters in Racket.
A T-expression is simply an S-expression in disguise.

Typically, T-expressions are enabled through
@racketmodname[scribble/base] or similar languages. But you can also
add T-expressions to an S-expression-based language using the
@racketmodname[t-exp] meta-language. For example,
...
Using @t-exp-racket[] is probably the easiest way to try the examples
in this chapter.

@;--------------------------------------------------------------------
@section{T-expressions at a glance}

To review @secref["how-to:reader"], the concrete syntax of @deftech{T-expressions}
is roughly
...
@t-exp-racket[] module. For example, running
...
Here are more examples of @tech{T-expressions}:
...
@tech{T-expressions}, where the syntax for @tech{T-expressions} is the same whether it's
in a @nonterm{text-body} context as in a Racket context.  A
@nonterm{text-body} that isn't a @tech{T-expression} is converted to a string
...
The command part of a @tech{T-expression} is optional as well. In that case,
the @tech{T-expression} is read as a list, which usually counts as a function
...
``@litchar["}|"]''.  Furthermore, any nested @tech{T-expressions} must begin
with a ``@litchar["|@"]''.
...
and @litchar{<>}s.)  With this extension, @tech{T-expression} syntax can be used as a
...
Finally, remember that @tech{T-expressions} are just an alternate form of
S-expressions. Identifiers still get their meaning, as in any
Racket code, through the lexical context in which they appear.
Specifically, when the above @tech{T-expression} appears in a Racket expression
...
@tech{T-expression} can have Racket punctuation prefixes, which will end up
...
@tech{T-expressions}, balanced comments can be confusing, since the open brace
...
If only the @nonterm{cmd} part of a @tech{T-expression} is specified, then the
...
characters with special meaning is @litchar["@"] for sub-@tech{T-expressions},
...
This applies to sub-@tech{T-expressions} too---the @litchar["@"] must be
...
text with @tech{T-expressions} freely, just prefix the @litchar["@"] if the
...
sub-@tech{T-expressions}, and the end-of-body marker should have these characters
...
as the Racket command part of a @tech{T-expression}.  The latter is used in this case
...
As noted above, there are two kinds of @tech{T-expression} comments: @litchar|{@;{...}}| is
a (nestable) comment for a whole body of text (following the same
rules for @tech{T-expressions}), and @litchar|{@;...}| is a line-comment.
...
The @tech{T-expression} syntax treats spaces and newlines in a special way is
...
Note that each T-expression is parsed to an S-expression that has its own
