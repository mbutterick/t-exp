...
@title[#:tag "reader-internals"]{T-expression reader internals}

@;--------------------------------------------------------------------
@section{Using the T-expression Reader}

You can use the reader via the @racket[t-exp] meta-language as described in
@secref["t-exp-lang"].

Note that the T-expression reader reads @tech{T-expressions} as S-expressions.  This
...
A common use of the T-expression reader is when using Scribble as a
documentation system for producing manuals. In this case, the manual
text is likely to start with

@racketmod[scribble/doc]

which installs the T-expression reader starting in ``text mode,'' wraps the
...
@tech{T-expressions}.  You can do this in a single command line:
...
The T-expression reader attaches properties to syntax objects.  These
...
In addition, the T-expression parser uses syntax properties to mark syntax
...
@;--------------------------------------------------------------------
@section[#:tag "t-exp-lang"]{Adding T-expressions to a language}

@defmodulelang[t-exp]{The @racketmodname[t-exp] language installs
...
For example, @racket[@#,hash-lang[] t-exp racket/base] adds @"@"-reader
...
@racketmodname[t-exp] attaches a run-time configuration annotation to
...
provides direct T-expression reader functionality for advanced needs.}

@; The `with-scribble-read' trick below shadows `read' and
@;  `read-syntax' with for-label bindings from the T-expression reader
...
Implements the T-expression reader using the readtable produced by
...
@item{@racket[command-char] --- the character used for @tech{T-expressions}.}

@item{@racket[command-readtable] --- determines the readtable that is
  extended for reading the command part of a @tech{T-expression}:
...
@item{@racket[datum-readtable] --- the readtable used for
  reading the datum part of a @tech{T-expression}:
...
@item{@racket[syntax-post-proc] --- function that is applied on
  ...
  control uses of @tech{T-expressions}, for example, making the command be the
  head of a list:
